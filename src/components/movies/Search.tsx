	...
	return (
		<>
			<section className="bg-light-purple rounded-2xl h-fit p-4">
				<input onChange={(e) => { handleChange(e.target.value) }} value={query} className="w-full p-4 text-lg text-white border-b-2 placeholder-white bg-transparent mb-4" type="text" name="query" placeholder="Search by movie name" />
				<ul className="h-96 overflow-x-hidden overflow-y-auto">
					{results.map((movie, index) => {
						return (
							<li key={index} className="mb-6">
								<a href={`/movie/${movie.id}`} className="relative m-auto w-5/6">
									<img className="rounded-xl" src={`https://image.tmdb.org/t/p/w500${movie.backdrop_path || movie.poster_path}`} />
