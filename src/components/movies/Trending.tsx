...
export const Trending = () => {
	...
	useEffect(() => {
		(async () => {
			try {
				const response = await fetch("https://api.themoviedb.org/3/trending/all/day?api_key=0544e732b88a2287b7288288339c30cf");
			...
			<section className="">
				<div className="bg-black flex flex-col items-center rounded-2xl h-fit font-Roboto p-4">
					<a href={`/movie/${topTrending.id}`} className="relative mb-10">
						<img className="rounded-xl w-full" src={`https://image.tmdb.org/t/p/w500${topTrending.backdrop_path}`} />
								...
								return (
									<li key={index} className="mb-6">
										<a href={`/movie/${movie.id}`} className="relative m-auto w-5/6">
											<img className="rounded-xl" src={`https://image.tmdb.org/t/p/w500${movie.backdrop_path}`} />
											<div className="opacity-0 hover:opacity-100 hover:bg-black/[.7] rounded-xl duration-300 absolute inset-0 z-10 flex justify-center items-center">
												<p className="text-xl text-white font-bold">
