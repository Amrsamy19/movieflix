			...
			<section>
				...
				<div className="grid gap-x-3 gap-y-6 grid-cols-4 w-full auto-cols-fr">
					{popularMovies.map((movie, index) => {
						return (
							<a href={`/movie/${movie.id}`} key={index} className="flex flex-col cursor-pointer hover:underline">
								<img className="rounded-xl w-full mb-6" src={`https://image.tmdb.org/t/p/w780${movie.poster_path}`} />
