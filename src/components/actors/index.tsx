    ...
    (async () => {
      try {
        const response = await fetch(
          "https://api.themoviedb.org/3/person/popular?api_key=0544e732b88a2287b7288288339c30cf"
        );
              ...
              actor.profile_path && (
                ...
                >
                  ...
                  <div className="flex flex-col justify-between">
                    <p className="text-md font-bold">{actor.name}</p>
                    <p className="text-md w-40 truncate font-medium text-gray-500/70">
                      {actor.known_for
                        .map((movie: IMovie) => movie.name || movie.title)
                        .join(", ")}
                    </p>
                  </div>
                </a>
